...
class ExternalWallThermalInsulationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = [
            ...
            'net_present_value', 'payback_period', 'discounted_payback_period', 'internal_rate_of_return',
...
class ExternalWallThermalInsulationCreateSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        existing = ExternalWallThermalInsulation.objects.filter(
            building=building, 
            user=user
        ).exists()
        ...
        if existing:
            raise serializers.ValidationError({
                'detail': 'Thermal insulation already exists for this building. You can only have one thermal insulation per building.'
            })
