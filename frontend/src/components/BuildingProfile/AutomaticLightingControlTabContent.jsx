...
const AutomaticLightingControlTabContent = ({
  ...
  useEffect(() => {
    ...
    const newAutoSave = setTimeout(() => {
      ...
      if (formData.lighting_area && formData.cost_per_m2 && formData.lighting_energy_savings) {
  ...
  const fetchAutomaticLightingControlData = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://127.0.0.1:8000/automatic_lighting_control/building/${buildingUuid}/`, {
        method: "GET",
        headers: {
    ...
    return (
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} sm={6}>
          <TextField
            fullWidth
            label={(translations.lightingArea || "Επιφάνεια φωτισμού") + " (m²) *"}
            type="number"
    ...
    return (
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} sm={6}>
          <TextField
            fullWidth
            label={(translations.lightingEnergySavings || "Εξοικονόμηση ενέργειας φωτισμού") + " (kWh/έτος) *"}
            type="number"
            value={formData.lighting_energy_savings}
            onChange={(e) => handleInputChange("lighting_energy_savings", e.target.value)}
            variant="outlined"
            required
    ...
    <div className="space-y-6 p-6 bg-gray-50 min-h-screen">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-gray-800">
                {translations.title || "Εγκατάσταση Συστήματος Αυτόματου Ελέγχου Φωτισμού"}
              </h2>
              <p className="text-gray-600 mt-1">
                {translations.subtitle || "Οικονομική ανάλυση συστήματος αυτόματου ελέγχου φωτισμού"}
              </p>
            </div>
          </div>
          <Button
      ...
      <div className="bg-white rounded-xl shadow-md">
        <Tabs
          ...
          <Tab label={translations.lightingSystemDataTab || "Στοιχεία Συστήματος Φωτισμού"} />
