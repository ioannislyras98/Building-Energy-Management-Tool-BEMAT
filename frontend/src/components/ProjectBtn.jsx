...
export default function ProjectBtn({
  ...
  const getSubmissionIcon = () => {
    if (is_submitted) {
      return (
        <FaCheckCircle className="text-green-500 size-4" title="Submitted" />
      );
    } else {
      ...
      <div className="progress-info text-[10px] sm:text-xs mt-1">
        <div className="flex justify-between">
          <span className="text-primary">
            ...
            {completion_status.systems_completed}/
            {completion_status.systems_total} (
            {completion_status.overall_systems_progress}%)
          </span>
        </div>
        <div className="flex justify-between">
          <span className="text-primary">
            {translations?.scenarios || "Scenarios"}:{" "}
            {completion_status.scenarios_completed}/
            {completion_status.scenarios_total} (
            {completion_status.overall_scenarios_progress}%)
          </span>
        </div>
      </div>
    );
