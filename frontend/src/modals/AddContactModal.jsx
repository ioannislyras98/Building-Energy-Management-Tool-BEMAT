...
function AddContactModalForm({ isOpen, onClose, onContactAdded, buildingUuid, params }) {
    ...
    $.ajax({
      ...
      error: function (error) {
        if (error.responseJSON && error.responseJSON.error) {
          ...
          setErrors({ general: params.errorGeneral || "Failed to add contact." });
        }
      },
    });
  };
      ...
      <div className="rounded-lg p-6 w-96 border-primary-light border-2 bg-white shadow-lg">
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex justify-between mt-6">
            <button type="button" onClick={onClose} className="close-modal">{params.cancel}</button>
            <button type="submit" className="confirm-button">{params.addContact}</button>
          </div>
        </form>
      </div>
    </div>
...
export default function AddContactModal({ isOpen, onClose, onContactAdded, buildingUuid }) {
  const { language } = useLanguage();
  const params = language === "en" ? english_text.AddContactModal : greek_text.AddContactModal;
