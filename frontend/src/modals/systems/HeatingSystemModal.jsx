...
function HeatingSystemModalForm({
  ...
  const handleSubmit = (e) => {
    ...
    const url = isEditMode
      ? `http://127.0.0.1:8000/heating_systems/update/${editItem.uuid}/`
      : "http://127.0.0.1:8000/heating_systems/create/";
    ...
    $.ajax({
      ...
      success: (response) => {
        console.log("Heating system saved:", response);
        onSubmitSuccess(response);
        resetForm();
        onClose();
      },
      error: (jqXHR) => {
        console.error("Error saving heating system:", jqXHR);
