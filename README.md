...
1. **"Failed to resolve import" errors ή Module cache προβλήματα**

   - 🎯 **Λύση**: `.\bemat.ps1` → επιλογή **"3"** (Clean Docker & Rebuild All)
   - Αυτό καθαρίζει όλο το Docker cache και κάνει rebuild από την αρχή
   - Διάρκεια: 3-5 λεπτά

2. **Containers δεν ξεκινούν**

   - Ελέγξτε ότι το **Docker Desktop** τρέχει
   - `.\bemat.ps1` → επιλογή **"4"** (System Diagnostics) για έλεγχο κατάστασης
   - Δοκιμάστε επιλογή **"2"** (Stop) και μετά **"1"** (Start)

3. **Ports σε χρήση**

   - `.\bemat.ps1` → επιλογή **"4"** (System Diagnostics)
   - Θα δείτε ποια ports είναι σε χρήση
   - Το script θα σας προτείνει να κλείσετε τις διεργασίες

4. **Frontend δεν φορτώνει ή βλέπετε cache προβλήματα**

   - `.\bemat.ps1` → επιλογή **"3"** (Clean Docker & Rebuild All)
   - **Hard refresh** στον browser: `Ctrl + Shift + R`
   - **Clear browser cache**: F12 → Application → Storage → Clear storage

5. **Docker errors ή "out of space"**

   - `.\bemat.ps1` → επιλογή **"3"** (Clean Docker & Rebuild All)
   - Καθαρίζει όλα τα παλιά images και volumes
...
| ----------------------- | -------------------------------------------- |
| Import errors           | `.\bemat.ps1` → **3** (Clean & Rebuild)      |
| Cache προβλήματα        | `.\bemat.ps1` → **3** + Browser hard refresh |
| Containers δεν ξεκινούν | `.\bemat.ps1` → **4** (Diagnostics)          |
| Ports σε χρήση          | `.\bemat.ps1` → **4** → Follow port cleanup  |
| Docker out of space     | `.\bemat.ps1` → **3** (frees up space)       |
| Γενικά προβλήματα       | `.\bemat.ps1` → **3** (nuclear option)       |
